...
class BinnedData(UnBinnedData):
    ...
    def get_binned_data(self, unbinned_data=None, output_name=None, \
            make_binning_plots=False, psichi_binning="local", psichi_old=False):
        ...
        - psichi_binning: 'galactic' for binning psichi in Galactic coordinates,
          or 'local' for binning in local coordinates. Default is local. 
        ...
        if psichi_old == False:
            if psichi_binning == 'local': # this was originally galactic, but for our purposes, we are essentially in galactic coordinates...
                psichi_axis = HealpixAxis(nside = self.nside, scheme = self.scheme, coordsys = 'galactic', label='PsiChi')
                coords = SkyCoord(l=self.cosi_dataset['Chi local']*u.deg, b=self.cosi_dataset['Psi local']*u.deg, frame = 'galactic')
            #if psichi_binning == 'local':
    ...
    def plot_binned_data(self, binned_data=None):
        ...
        # Make plots:
        plot_list = [energy_plot,phi_plot,psichi_plot]
