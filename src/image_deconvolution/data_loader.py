...
from cosipy.data_io import create_cross_sec
...
class DataLoader(object):
    ...
    @classmethod
    def load_from_filepath(cls, event_tra_filepath = None, event_yaml_filepath = None, rsp_filepath = None):
        
        new = cls()
        
        new.set_event_from_filepath(event_tra_filepath, event_yaml_filepath)
        #new.set_rsp_from_filepath(rsp_filepath) #this is commented out until we have full detector response
        
        return new

    def set_event_from_filepath(self, tra_filepath, yaml_filepath):
        ...
        print(f'... loading event data from {tra_filepath}')
        
        event_unbinned = UnBinnedData(self._event_yaml_filepath)
        event_unbinned.read_tra(output_name="/home/eshread/nn-response-approximation/TestSource.520.unbinned")
        
        unbinned_hdf5_filepath = "/home/eshread/nn-response-approximation/TestSource.520.unbinned.hdf5"
        self._event_unbinned_hdf5_filepath = unbinned_hdf5_filepath
        
        event_binned = BinnedData(self._event_yaml_filepath)
        event_binned.get_binned_data(unbinned_data=self._event_unbinned_hdf5_filepath, 
            output_name="/home/eshread/nn-response-approximation/TestSource.520.binned")

        binned_hdf5_filepath = "/home/eshread/nn-response-approximation/TestSource.520.binned.hdf5"
        self._event_binned_hdf5_filepath = binned_hdf5_filepath
        
        event_binned.load_binned_data_from_hdf5(self._event_binned_hdf5_filepath)
    ...
    def set_rsp_from_filepath(self, filepath):
        ...
        with h5py.File('/home/eshread/nn-response-approximation/TestSource.102.response.hf5', 'w') as hf:
            hf.create_dataset('DRM', data=y)

        self._rsp_filepath = '/home/eshread/nn-response-approximation/TestSource.102.response.hf5'
        f.close()
