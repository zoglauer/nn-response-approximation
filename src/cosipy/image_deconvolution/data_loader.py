...
class DataLoader(object):
    ...
    @classmethod
    def load_from_filepath(cls, event_tra_filepath = None, event_unbinned_yaml_filepath = None, rsp_filepath = None):

        new = cls()

        new.set_event_from_filepath(event_tra_filepath, event_unbinned_yaml_filepath)
        
        new.set_rsp_from_filepath(rsp_filepath)

        return new

    def set_event_from_filepath(self, tra_filepath, unbinned_yaml_filepath):
        ...
        ----------
        tra_filepath : str
            File path of .tra file for event data.
        yaml_filepath : str
            File path of yaml file to read the .tra file.
        """

        self._event_tra_filepath = tra_filepath
        self._unbinned_event_yaml_filepath = unbinned_yaml_filepath

        print(f'... loading event from {tra_filepath} and {unbinned_yaml_filepath}')

        event_unbinned = UnBinnedData(self._unbinned_event_yaml_filepath)
        event_unbinned.read_tra(output_name="/home/eshread/nn-response-approximation/TestSource.185.unbinned")
        
        print("... Done ...")

        """
        Option to bin data according to parameters in yaml file. In this case, add an additional argument to this function 
        for the yaml file for the binned data, and add similar argument to load_from_filepath.
        
        unbinned_hdf5_filepath = "/home/eshread/nn-response-approximation/TestSource.185.unbinned.hdf5"
        self._unbinned_event_hdf5_filepath = unbinned_hdf5_filepath
        
        event_binned = BinnedData(self._binned_event_yaml_filepath)
        event_binned.get_binned_data(unbinned_data=self._unbinned_event_hdf5_filepath, 
            output_name="/home/eshread/nn-response-approximation/TestSource.185.binned")

        binned_hdf5_filepath = "/home/eshread/nn-response-approximation/TestSource.185.binned.hdf5"
        self._binned_event_hdf5_filepath = binned_hdf5_filepath
        
        event_binned.load_binned_data_from_hdf5(self._binned_event_hdf5_filepath)

        self.event_dense = event_binned.binned_data.to_dense()
        self.event_sparse = event_binned.binned_data.to_sparse()
        """
    ...
    def set_rsp_from_filepath(self, filepath):
        ...
        """
        Load response matrix from filepath
        ...
        # Convert to an HF5 file
        # Read data from input file
        with open(filepath, 'rb') as f:
            data_dict = pickle.load(f)
            y = data_dict.get('y')
        
        with h5py.File('/home/eshread/nn-response-approximation/FlatContinuumIsotropic.inc520.id1.tra.hf5', 'w') as hf:
            hf.create_dataset('data', data=y)

        self._rsp_filepath = '/home/eshread/nn-response-approximation/FlatContinuumIsotropic.inc520.id1.tra.hf5'

        # cannot apply full_detector_response to rsp in its current format
    ...
    def _check_axis_consistency(self):
        ...
        axis_name = ['Em', 'Phi', 'PsiChi'] # 'Time' should be changed if one uses the scat binning.
